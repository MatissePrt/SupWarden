...
export const deleteTrousseau = async ({ id }) => {
    try {
        const token = Cookies.get('userInfo');
        const userInfo = token ? JSON.parse(token) : null;
        const authToken = userInfo ? userInfo.token : '';
...
export const respondToInvitation = async (trousseauId, response) => {
    try {
        ...
        return { success: false, message: 'Erreur de réponse à l\'invitation' };
    }
};
...
export const getUserInvitations = async () => {
    try {
        ...
        return { success: false, message: 'Erreur de récupération des invitations' };
    }
};
...
export const addElement = async (trousseauId, elementData) => {
    try {
        const token = Cookies.get('userInfo');
        const userInfo = token ? JSON.parse(token) : null;
        const authToken = userInfo ? userInfo.token : '';

        const response = await fetch(`http://localhost:5000/api/elements/${trousseauId}/elements`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`,
        ...
        return { message: 'Erreur d\'ajout de l\'élément' };
    }
};

export const getElementsByTrousseau = async (trousseauId) => {
    try {
        const token = Cookies.get('userInfo');
        const userInfo = token ? JSON.parse(token) : null;
        const authToken = userInfo ? userInfo.token : '';
        ...
        return { message: 'Erreur de récupération des éléments' };
    }
};
...
export const getElementDetails = async (elementId) => {
    try {
        ...
        const response = await fetch(`http://localhost:5000/api/elements/details/${elementId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`,
        ...
        console.error('Erreur de récupération des détails de l\'élément:', error);
        return { message: 'Erreur de récupération des détails de l\'élément' };
    }
};